...
// ============================================================
//   STRUCTS
//   (MataKuliah dipindah ke atas karena Mahasiswa & Dosen
//    sekarang menyimpan POINTER ke MataKuliah, bukan index int)
// ============================================================
struct MataKuliah {
    int id;
    string nama_mk;
...
// Pencarian login mahasiswa berbasis pointer: mengembalikan
// alamat data mahasiswa yang cocok, atau nullptr jika tidak ada.
Mahasiswa* cariLoginMahasiswa(const string &nim, const string &pass) {
...
// Pencarian login dosen berbasis pointer
Dosen* cariLoginDosen(const string &usn, const string &pass) {
    Dosen* p = data_dsn;
...
void mhs_pilih_mk(Mahasiswa* mhs) {
    ...
    if (pil >= 1 && pil <= MAKS_MK) {
        MataKuliah* mk = list_mk + (pil - 1); // pointer ke matkul terpilih
...
void mhs_absen(Mahasiswa* mhs) {
    ...
    Absensi* baru = &data_absen[jml_absen]; // pointer ke slot absensi baru
    baru->nim = mhs->nim;
...
void riwayatDosen(Dosen* dsn) {
    ...
    Absensi* p = data_absen;
    for (int i = 0; i < jml_absen; i++, p++) {
        if (p->matkul == matkulDosen->nama_mk && p->kelas == kelasDosen) {
            ditemukan = true;
            cout << "Tanggal  : " << p->tgl << " (" << p->jam << ")\n";
            cout << "NIM      : " << p->nim << "\n";
            cout << "Nama     : " << p->nama << "\n";
...
void hapusMahasiswa() {
    ...
    Mahasiswa* target = data_mhs + (pil - 1);   // pointer ke data yang dihapus
    Mahasiswa* akhir  = data_mhs + (jml_mhs - 1);

    for (Mahasiswa* q = target; q < akhir; q++) {
        *q = *(q + 1); // geser data via pointer
    }
    jml_mhs--;
...
void hapusMatkulMahasiswa() {
    ...
    Mahasiswa* mhs = data_mhs + (pil - 1); // pointer ke mahasiswa terpilih
...
void menuUtama() {
    ...
    while (true) {
        system("cls");
        cetak_banner();
        cout << "  Data Mhs: " << jml_mhs << "/" << MAKS_MHS << "  |  Panah: navigasi  |  Enter: pilih" << endl;
        cout << "---------------------------------------------------------------" << endl;
                ...
                while (true) {
                    ...
                    if (menu_awal == 1) {
                        ...
                        if (jml_mhs < MAKS_MHS) {
                            ...
                            Mahasiswa* baru = data_mhs + jml_mhs; // pointer ke slot baru
                    ...
                    if (menu_awal == 1) {
                        if (jml_dsn < MAKS_DSN) {
                            ...
                            if (cek) {
                                ...
                                Dosen* baru = data_dsn + jml_dsn; // pointer ke slot dosen baru
